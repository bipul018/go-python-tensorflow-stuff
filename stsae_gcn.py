...
def entry_point(in_channels = 3, hidden_channels= 64, num_classes= NUM_CLASSES, num_frames= 20, LR = 0.001, Epochs =50):
    dataset = Yoga3DDataset(read_meta_data())
    train_dataset, val_dataset, test_dataset = train_val_test_split(dataset)
    train_loader, val_loader, test_loader = create_data_loaders(train_dataset, val_dataset, test_dataset)
...
class FrameSampler():
    def __init__(self, cxt_size, landmark_count=33, dims=3):
        self.cxt_size = cxt_size
        self.tensor = torch.zeros([cxt_size, landmark_count, dims])
        self.last_pred = ""
        self.last_prob = 0

    def from_other(self, another_sampler, cxt_size):
        self.tensor = torch.zeros([cxt_size, landmark_count, dims])
    ...
    def get_pred_str(self):
        return f'{self.last_pred}({self.last_prob*100:.1f}%)'

sampler = FrameSampler(cxt_size = 20)
